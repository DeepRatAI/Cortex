...
class Settings(BaseSettings):
    ...
    rate_limit_qpm: int = Field(
        default=120, description="Queries per minute allowed per process"
    )
    conversation_max_turns: int = Field(
        default=5, description="Max previous turns to include in prompt context"
    )
    api_key: str | None = Field(
        default=None, description="API key to protect endpoints; disabled if empty"
    )
    cors_origins: str = Field(
        default="*", description="Comma-separated list of allowed CORS origins"
    )
    https_enabled: bool = Field(
        default=False, description="Enable HTTPS-only headers (CSP/HSTS)"
    )
    ...
    enable_streaming: bool = Field(
        default=False, description="Enable SSE /chat/stream endpoint"
    )
    log_level: str = Field(
        default="INFO", description="Log level: DEBUG|INFO|WARNING|ERROR"
    )
    max_input_tokens: int = Field(
        default=2048, description="Max input tokens per request"
    )
    max_output_tokens: int = Field(
        default=512, description="Max output tokens per request"
    )
    rate_limit_burst: int = Field(
        default=0, description="Additional burst capacity per key"
    )
    rate_limit_window_seconds: int = Field(
        default=60, description="Sliding window in seconds"
    )
